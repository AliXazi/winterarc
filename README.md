...
- **Daily protocol:** Check tasks → polygon +9/arc-days per check (e.g., 90d +0.10), progress bar + pill update instantly via `refreshUI()`. 100% stamps `last_100_date` and increments streak (consecutive days); miss resets on next load.
